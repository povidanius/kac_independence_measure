...
class KacIndependenceMeasure(nn.Module):
    ...
    def reset(self):
        self.b = Variable(torch.rand(self.dim_y), requires_grad=True)

        if self.input_projection_dim > 0:
            self.a = Variable(torch.rand(self.input_projection_dim), requires_grad=True)
            ...
            self.a = Variable(torch.rand(self.dim_x), requires_grad=True)
