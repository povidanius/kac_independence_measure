...
\subsection{Interpretation and connection to other approaches}
\textbf{Interpretation as covariance}. Since \eqref{eq:kim} can be reformulated as
=======
...
<<<<<<< HEAD
\textbf{$f$-divergence-based measures} Recall that the $f$-divergence between two probability distributions $P$ and $Q$ is defined as $D_{f}(P|Q) = \mathbb{E}_{Q} f(\frac{P}{Q})$, and corresponding analogue of mutual information - $D_{f}(P_{x,y}|P_{x}P_{y})$
