...
By \textit{Levy continuity theorem} \cite{LCT} ECF weakly converges CF.
Uniqueness theorem states that two random variables $X$ and $Y$ have the same distribution if and only if their CF's are identical.
...
%Let us correspondingly denote empirical $\widehat{\Delta}_{X,Y}(\alpha, \beta) := \widehat{\phi}_{X,Y}(\alpha,\beta) - \widehat{\phi}_{X}(\alpha) \widehat{\phi_{Y}}(\beta)$.
...
Algorithm~\ref{alg:estimator_computation} requires to initialise $\alpha$ and $\beta$, select stopping criteria, and optimiser. In our implementation we use uniform initialisation of parameters, stopping criteria defined in number of iterations, and decoupled weight decay regularization optimizer~\cite{Loshchilov2019DecoupledWD}. 
...
In each experiment we train classifier $30$ times with randomly splitted training and testing data set of aforementioned sizes. The average accuracies reported in Table~\ref{table:regularisation_classification_accuracies}, that application~\eqref{eq:regularizer1} slightly (but with statistical significance) increased classification accuracy.
