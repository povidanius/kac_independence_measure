...
\paragraph{Comparison with distance correlation} We also evaluated distance correlation~\cite{dcor} on the same generated samples of data.From Figure.~\ref{fig:experiments_simulation_dcor} we see that as data dimensionality grows, for independent data, the values of measure not is significantly larger thatn zero, but it also grows (although is smaller comparing to the dependent data case), which demonstrates that distance correlation is vulnerable to curse of dimensionality. On the other side, KacIM even for larger dimensions osculates near zero for independent data, and significantly deviated from zero for dependent one.

\begin{figure}%
	\centering
	\includegraphics[scale=0.40]{../experiments/basic_demonstration/dependence_detection_dcor_dim.png}%
	\caption{The dimension of data is on the $x$ axis, and on $y$ axis is evaluation of distance correlation for independent (blue), and dependent(orange) data.}
