...
In feature extraction experiments we will use a set of classification data sets from OpenML~\cite{OpenML2013}.  We conduct linear feature extraction by seeking 
...
\noindent Afterwards, feature extraction is conducted by $f = W^{*}x$ and these features are used as the inputs to logistic regression classifier.
...
\begin{table}	
	...
	\begin{tabular}{ |p{3cm}|p{2.0cm}|p{1.2cm}|p{1.7cm}|p{1.2cm}|  }
		...
		\hline
	\end{tabular}
	\caption{Classification accuracies, trained on KacIM-based features.}
