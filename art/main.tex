...
Since \eqref{eq:kim} can be reformulated as
\begin{equation}
...
In special case when both $X$ and $Y$ are zero mean Gaussian random vectors we have:
\begin{equation}
...
In each experiment we train classifier $20$ times with randomly splitted training and testing data set of aforementioned sizes. The average accuracies are reported in Table~\ref{table:regularisation_classification_accuracies}.
...
\begin{table}	
	...
	\begin{tabular}{ |p{4cm}|p{3cm}|}
		...
		\hline
		Without regularisation   &    0.8727 \\		
		\hline
		With regularisation  &   \textbf{0.8775} \\		
		\hline
	\end{tabular}
	\caption{Classification accuracy comparison of regularised and not regularised model. Bold text indicates that model with regulariser was more accurate (Wilcoxon's signed rank test~\cite{Wilcoxon1992}, 20 runs, $p$-value threshold $0.06$))}
