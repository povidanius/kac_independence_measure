...
kim_xz = KacIndependenceMeasure(512, 32, lr=0.0005,  input_projection_dim=32, weight_decay=0.01,device=device) #0.007
kim_zy = KacIndependenceMeasure(32, 2, lr=0.0005, weight_decay=0.01,device=device) #0.007
...
model.fc[0].register_forward_hook(get_activation('fc_0'))
...
for epoch in range(number_of_epoch):
    ...
    while epoch_finished == False:
        ...
        if mode == REGULARIZER:
            ...
            reg_xz = kim_xz.forward(activation['fc_0'].squeeze(), bottleneck1, update=True)
            #breakpoint()
            print("Mode: REGULARIZER")
            #print(iteration % optimize_kac_every_iters)
            print("reg_xz {}".format(reg_xz))
            #print("bottleneck: {}, y {}".format(bottleneck.shape, y.shape))
            dep_history.append(reg_xz.detach().cpu().numpy())
            if reg_xz.item() > 0.15:
                mode = LOSS
                internal_iter = 0
                continue
            ...
            if use_regularization:
                ...
                reg_xz = kim_xz.forward(activation['fc_0'].squeeze(), bottleneck1, update=False)
                print("loss {},  reg_xz {}".format(loss,  reg_xz))
                loss = loss - reg_alpha*reg_xz # + reg_alpha * reg 
                if reg_xz.item() < 0.05:
                    mode = LOSS
                    internal_iter = 0
                    continue 
...
with open("./8result_{}_{}.txt".format(use_regularization, reg_alpha),"a") as f:
