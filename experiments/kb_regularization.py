...
model.fc[3].register_forward_hook(get_activation('bottleneck'))
#model.fc[2].register_forward_hook(get_activation('bottleneck0'))
...
for epoch in range(number_of_epoch):
    ...
    while epoch_finished == False:
        ...
        optimizer.zero_grad()             
        pred1 = model(x1)
        bottleneck1 = activation['bottleneck'].squeeze()
        y_1 = torch.nn.functional.one_hot(label1).float()
        pred2 = model(x2)     
        bottleneck2 = activation['bottleneck'].squeeze()
        ...
        z1prim = z1.clone().detach().to(device)
        z2prim = z2.clone().detach().to(device)
        reg0 = kim.forward(z1prim, z2prim, update=True)
        reg1 = kimfy.forward(torch.cat((bottleneck1, bottleneck2),dim=0).clone().detach().to(device), torch.cat((y_1, y_2),dim=0).clone().detach().to(device), update=True)
